...
if 1==1:
    ...
    if len(few_class_label)>1:
        pair_sample_test = np.array(pair_sample_test).reshape(-1,2*feature_number)
        pair_sample_train = np.array(pair_sample_train).reshape(-1,2*feature_number)
        model_ind = 4

        model = Sequential()
        model.add(Dense(128, activation='relu', input_dim=2*feature_number))
        ...
        model_name = 'direct_%02d_trained_%s_%d_model.h5'%(model_ind, data_type,feature_number)
        ...
        for i in range(pair_sample_test.shape[0]):
            ...
            pearsonr_list.append(pearsonr(item[:feature_number], item[feature_number:])[0])
...
data = data.reshape(-1, feature_number)
print(data.shape[0])
...
for pair_id in range(10):
    ...
    for batch in range(5):
        ...
        model = Sequential()
        model.add(Dense(128, activation='relu', input_dim=feature_number))
        model.add(Dropout(0.25))
        model.add(Dense(64, activation='relu'))
        model.add(Dropout(0.5))
        model.add(Dense(32, activation='relu'))
        ...
        model_name = 'dense_%02d_trained_model_%s_%d.h5'%(model_ind,method,feature_number)
...
for batch in range(5):
    ...
    file = open('/home/chenxiaoyang/program/scmap/Data/processed/%s/sample_%s_%d_%d_all.txt' % (test_name,method, batch,feature_number), 'w')
    file.write("%f\n%f\n%f" % (A, B, assigned_rate))