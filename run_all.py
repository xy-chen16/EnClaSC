...
if 1==1:
    ...
    if len(few_class_label)>1:
        ...
        for i in range(normalized_pred_res_each_cell.shape[0]):
            ...
            if np.std(ref_label_scores) == 0 or 0.5 > np.max(ref_label_scores):
...
for batch in range(5):
    ...
    file = open('/home/chenxiaoyang/program/scmap/Data/processed/%s/sample_%s_%d_all.txt' % (test_name,method, batch), 'w')
    file.write("%f\n%f\n%f" % (A, B, assigned_rate))
    pred_label = []
    for i in range(len(pred)):
        if pred[i] == 100:
            pred_label.append("unassigned")
        else:
            pred_label.append(list(LE.inverse_transform([int(pred[i])]))[0])
    hkl.dump(pred_label, '/home/chenxiaoyang/program/scmap/Data/processed/%s/sorted_residual/pred_%s_label_%d_v1.0.hkl'%(test_name,method,batch))