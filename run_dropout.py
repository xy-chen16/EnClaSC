...
for dropout in range(10):
    dropout_rate = float(dropout)/float(10)
    ...
    data_nonzero = np.nonzero(data)
    data_nonzero_length = data_nonzero[0].shape[0]
    data_nonzero_index = np.arange(data_nonzero_length)
    np.random.shuffle(data_nonzero_index)
    data[data_nonzero[0][data_nonzero_index[0:int(data_nonzero_length*dropout_rate)]],data_nonzero[1][data_nonzero_index[0:int(data_nonzero_length*dropout_rate)]]]=0
    data_nonzero = np.nonzero(data_test)
    data_nonzero_length = data_nonzero[0].shape[0]
    data_nonzero_index = np.arange(data_nonzero_length)
    np.random.shuffle(data_nonzero_index)
    data_test[data_nonzero[0][data_nonzero_index[0:int(data_nonzero_length*dropout_rate)]],data_nonzero[1][data_nonzero_index[0:int(data_nonzero_length*dropout_rate)]]]=0
    if 1==1:

        few_class_label = []
        for i in range(len(pd.value_counts(label_train))):
            if pd.value_counts(label_train)[i] < 0.003*len(label_train):
                few_class_label.append(pd.value_counts(label_train).index[i])
    ...
    for pair_id in range(10):
        X_train, _, Y_train, _ = train_test_split(data, y_train, test_size=1 - random_rate, random_state=0)
        ...
        for batch in range(5):
            ...
            batch_size = 16
            epochs = 30
            save_dir = os.path.join('/home/chenxiaoyang/program/scmap/Data/processed/%s/'%test_name, '%d'%pair_id,'%d'%batch)
    ...
    for batch in range(5):
        ...
        for i in range(k):
            pred_label_list = []
            for j in range(10):
                pred_label_list.append(pred_label_names[j * k + i])
            if (pred_label_list.count(max(pred_label_list, key=pred_label_list.count)) > 4):
                pred_names_list.append(max(pred_label_list, key=pred_label_list.count))
            else:
                pred_names_list.append(100)
        label = y_test.tolist()
        ...
        file = open('/home/chenxiaoyang/program/scmap/Data/processed/%s/sample_%s_%d_dropout_rate=%.2f.txt' % (test_name,method, batch,dropout_rate), 'w')
        file.write("%f\n%f\n%f" % (A, B, assigned_rate))