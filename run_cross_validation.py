...
data_all  = hkl.load('/home/chenxiaoyang/program/scmap/Data/processed/%s/data_test_%d_sample_%s.hkl'%(test_name, feature_number,method))

label_train_all  = hkl.load('/home/chenxiaoyang/program/scmap/Data/processed/%s/sorted_%s/data_test_label.hkl'%(test_name,data_type))
label_train_all = np.array(label_train_all)
skf = StratifiedKFold(n_splits=5)
for train_index, test_index in skf.split(data_all, label_train_all):
    data ,label_train = data_all[train_index],label_train_all[train_index]
    data_test,label_test = data_all[test_index],label_train_all[test_index]
    ...
    if 1==1:
        ...
        if len(few_class_label)>1:
            ...
            lgb_train = lgb.Dataset(pair_sample_train,pair_label_train)
            params = {
                'task': 'train',
            ...
            y_prob = gbm.predict(pair_sample_test, num_iteration=gbm.best_iteration)
    ...
    for pair_id in range(10):
        ...
        batch_size = 16
        epochs = 30
        save_dir = os.path.join('/home/chenxiaoyang/program/scmap/Data/processed/%s/'%test_name, '%d'%pair_id)
    ...
    file = open('/home/chenxiaoyang/program/scmap/Data/processed/%s/sample_%s_%d_cross_validation.txt' % (test_name,method,all_num), 'w')
    file.write("%f\n%f\n%f" % (A, B, assigned_rate))
    all_num = all_num+1